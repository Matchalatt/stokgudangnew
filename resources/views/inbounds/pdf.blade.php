...
<body>
    ...
    <div class="header">
        <h2>Laporan Riwayat Barang Masuk (Inbound)</h2>
    ...
    <table class="report-table">
        <thead>
            <tr>
                ...
                <th style="width: 85px;" class="text-center">Tanggal Masuk</th>
        ...
        <tbody>
            @forelse($inbounds as $index => $inbound)
                <tr>
                    ...
                    <td class="text-center">{{ \Carbon\Carbon::parse($inbound->tanggal_fisik)->format('d M Y') }}</td>
                    <td class="font-mono">{{ $inbound->reference ?? '-' }}</td>
                    <td><strong>{{ $inbound->item->nama ?? 'Barang Tidak Ditemukan' }}</strong></td>
                    <td class="text-right"><span class="badge-success">+{{ $inbound->qty_base }}</span> <small>{{ $inbound->item->base_unit ?? '' }}</small></td>
                    <td>{{ $inbound->keterangan ?? '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="text-center" style="padding: 20px; color: #777;">Tidak ada riwayat transaksi barang masuk dalam periode ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
